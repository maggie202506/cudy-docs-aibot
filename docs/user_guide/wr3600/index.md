...
- View the router's appearance—[front](overview.md#front-panel) and [back](overview.md#back-panel).
- View the [interaces](overview.md#back-panel) layout and description. 
- Check the [LED](overview.md#front-panel) status and indication.

## Connection 
- [Position](connection.md#position-the-router) the router properply before connection.
- Finish the [hardware connection](connection.md#connect-the-router) of the router.

## Quick Setup
- Set up the router to the Internet with [web-based wizard](quick_setup.md#quick-setup).
- Set up the router to the Internet via [Cudy App](quick_setup.md#cudy-app-setup).

---

## Management
### I. System Status
- View the status of the router, including [*Device*](status.md#device), [*WAN*](status.md#wan), [*LAN*](status.md#lan), [*Wireless 2.4G/5G*](status.md#wireless-24g), [*IPv6*](status.md#ipv6), [*VPN*](status.md#vpn), [*DHCP*](status.md#dhcp-server) and [*System*](status.md#system).

### II. General Settings
- [WAN Mode](wan.md): is to set your Internet connection. Select your Internet connection type from the drop-down list of Protocol. Follow the instructions on the page to complete the configuration.
- [Wireless](wireless.md): is to customize the router's wireless settings, including SSID and password, mode, channel, channel width and transmit power, and so on.
- [VPN](vpn.md): namely, Virtual Private Network, helps you access Internet resources remotely, securely, and privately through tunneling technology. When you access the Internet, VPN encrypts your personal information and hides your IP address from the public. For VPN users, it looks like the devices are directly connected. 
...
- [Firmware](firmware.md): Update the router's firmware to be the latest version.
...
- [System Log](diagnostic_tools.md#system-log): Track all the router behaviors. When the router does not work normally, download the system log and send it to our Technical Support for troubleshooting.